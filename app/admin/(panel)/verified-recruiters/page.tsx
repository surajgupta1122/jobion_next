...
export default function VerifiedRecruitersPage() {
  ...
  const load = async () => {
    try {
      setLoading(true);
      setError(null);
      const { data } = await api.get("/admin/recruiters?status=verified");
  ...
  return (
    <div className="space-y-5">
      <Header
        ...
        subtitle={`Total ${rows.length} verified recruiters`}
        onRefresh={load}
      />
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div className="px-4 py-3 bg-blue-600 text-white">
          <div className="font-semibold">Verified Recruiters ({loading ? "-" : rows.length})</div>
          <div className="text-xs opacity-90">Recruiters that have been approved for verification</div>
        </div>
                  ...
                  <tr key={r.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <button className="px-3 py-1.5 rounded-md border border-amber-200 text-xs font-semibold text-amber-700 hover:bg-amber-50">
                          Unverify
                        </button>
                      </div>
                    </td>
