...
export default function ClosedJobsPage() {
  ...
  const load = async () => {
    try {
      setLoading(true);
      setError(null);
      const { data } = await api.get("/admin/jobs?status=closed");
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Total {loading ? "-" : rows.length} closed jobs
          </p>
        </div>
        <button
          onClick={load}
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div className="px-4 py-3 bg-gray-800 text-white">
          <div className="font-semibold">Closed Jobs ({loading ? "-" : rows.length})</div>
          <div className="text-xs opacity-90">Jobs closed by admin or expired</div>
        </div>

        <div className="p-4">
          <div className="text-sm text-gray-600">
            View them in <a className="text-blue-600 underline" href="/admin/jobs">Jobs</a> → Closed tab.
          </div>
        </div>
      </div>
    </div>
